...
public class LoopExercise {
    public static void main(String[] args) {
      ...
      System.out.println("Product of evenSum and oddSum: " + product);
      ...
      System.out.println("arr10: " + String.join(", ", arr10));
      ...
      for (int i = 0; i < s11.length(); i++) {
        if (Character.isUpperCase(s11.charAt(i))) { // ! Character.isUpperCase()
          uppercaseCount++;
        }
      }
      ...
      System.out.println("Total score: " + totalScore); // Total score: 11
      ...
      // Print: [0.3, 0.4, 0.7]
      // Use BigDecimal
      // ! float + float = float, but with precision issue
      for (int i = 0; i < arr16.length; i++) {
        arr16[i] = new BigDecimal(Float.toString(arr16[i])) //
      ...
      System.out.println("count name=" + targetCount);
      ...
      // Print: "49280"
      String s19 = "40289";
      char[] chs = s19.toCharArray(); // O(n)
      char maxDigit = 0; // initial
      char minDigit = 9; // initial
      int maxIndex = 0, minIndex = 0;
      for (int i = 1; i < chs.length; i++) { // O(n)
        if (chs[i] > maxDigit) {
          maxDigit = chs[i];
          maxIndex = i;
        }
      ...
      // Swap max and min digits
      chs[maxIndex] = minDigit;
      chs[minIndex] = maxDigit;
